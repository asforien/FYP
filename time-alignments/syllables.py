...
initial_check = {'b', 'f', 'g', 'h', 'k', 'm', 'p', 's', 't', 'w'}
		...
		try:
			...
			if lexicon_entry[:-1] in syllabic_consonant_strs:
				...
				if lexicon_entry[0] not in vowel_strs and segments[segment_idx][2] not in vowels:
					...
					segment = segments[segment_idx]
					# sanity check
					if (lexicon_entry[0] in initial_check or phone_repr[segment[2]] in initial_check) and lexicon_entry[0] is not phone_repr[segment[2]]:
						num_errors += 1
						syllables.append([fileName, '?', '?', symbol, '?', '?'])
						continue
					segment_idx += 1
					start = segment[0]
					syllable_repr += phone_repr[segment[2]]
...
with open('syllable_allignments.csv', 'w') as outputFile:
